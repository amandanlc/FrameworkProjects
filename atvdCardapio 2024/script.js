...
//pra não precisar recarregar a página toda hora
function submit(){
...
function input(){
    comida.focus(); //indica que vai estar em foco, o input será o primeiro a ser 
                    //digitado e ele fica mais escuro para chamar atenção para o inputt
    comida.addEventListener("keyup", escreveu); 

    function escreveu(evento /*variavel qualquer para pegar todos os itens do keyup*/){
         console.log(evento)
        if(evento.key == "Enter" && comida.value != ' '){
            insertName(evento.target.value);
            //o value do target é a palavra como todo
        }
    }//ponto de parada keyup

    function insertName(newName) { 
        globalComida.push(newName); //adiciona ao vetor global comida
        //alert(globalComida)
        adicionar(newName); //recebe a comida, identifica o dia e adiciona
    }
...
function apagar(index){
    ...
    document.getElementById(id).style.display = "none" //apaga visualmente
    var indice = index - 1;
